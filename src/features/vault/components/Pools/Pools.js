...
import { useFetchCoinkbuyback } from 'features/vault/redux/fetchCoinkBuyback';
...
export default function Pools() {
  ...
  const { coinkbuyback, fetchCoinkbuyback, fetchCoinkbuybackDone } = useFetchCoinkbuyback();
  ...
  useEffect(() => {
    fetchCoinkbuyback();
    const id = setInterval(fetchCoinkbuyback, FETCH_INTERVAL_MS);
    return () => clearInterval(id);
  }, [fetchCoinkbuyback]);
  ...
  const chainCoinkbuyback =
    fetchCoinkbuybackDone && chainNameLowercase in coinkbuyback
      ? coinkbuyback[chainNameLowercase].buybackUsdAmount
      : undefined;
    ...
    <Grid container className={classes.container}>
      ...
      <Grid item xs={6}>
        <div className={classes.tvl}>
          ...
          {fetchCoinkbuybackDone && chainCoinkbuyback && (
            <span className={classes.text}>
              {t('Vault-CoinkBuyback', { amount: formatGlobalTvl(chainCoinkbuyback) })}
            </span>
          )}
