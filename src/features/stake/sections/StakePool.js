...
export default function StakePool(props) {
  ...
  useEffect(() => {
    ...
    if (pools[index].token === 'pigAutoWbnbFixed') {
    ...
    <Grid container>
      ...
      >
        ...
        <Grid item xs={6} sm={6} md={3}>
          <Typography className={classes.title}>{`${
            ...
            pools[index].token === 'pigAutoWbnbFixed' ? 'pigAutoWBNB' : pools[index].token
          }`}</Typography>
      ...
      >
        <Grid item xs={12} sm={4}>
          ...
          <Typography className={classes.subtitle}>
            {t('Stake-Total-Value-Locked', { pigToken: pools[index].token })}
          </Typography>
        </Grid>
        <Grid item xs={12} sm={4}>
